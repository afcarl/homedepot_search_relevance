...
xgb_params = {'num_round': 2000, 'colsample_bytree': 0.5, 'silent': 1, 'nthread': 3,
             'min_child_weight': 10, 'subsample': 0.5, 'eta': 0.004, 'fit_const': 0.5, 'objective': 'reg:linear',
             'max_depth': 10, 'gamma': 0}
featureData = clf.fit_transform(train)
featureData.fillna(-1, inplace=True)
print 'feature sucessful'
# print featureData.columns
train = featureData
print train.shape
print y_train
# dtrain = xgb.DMatrix(train, label= y_train.values)
# xgb.cv(xgb_params, dtrain,num_boost_round=10, nfold=5, metrics={'rmse'}, show_progress=True)
#
# # param_grid ={'xgbr__silent': [1], 'xgbr__nthread': [3], 'xgbr__eval_metric': ['rmse'], 'xgbr__eta': [(x+1)*0.002 for x in range(20)],
# #              'xgbr__max_depth': [5,6,7,8,9,10], 'xgbr__num_round': [800,900,1200,1400,1600,1800,2000], 'xgbr__fit_const': [0.4,0.5,0.6,0.7],

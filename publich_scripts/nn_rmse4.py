#-*-coding:utf-8-*-
'''
this version is user the nn in keras to do the classification
'''
import time
...
#import enchant
import random
from neuralNetwork import Lstm_burness
random.seed(2016)
...
lstm = Lstm_burness(neurons_list=[300,3],batch_size=50, nb_epoch=20)
...
clf = pipeline.Pipeline([
    ...
    ('lstm', lstm)])
param_grid ={'lstm__neurons_list': [[1024,512]], 'lstm__batch_size':[50],'lstm__nb_epoch':[20]}
...
print y_pred.shape
pd.DataFrame({"id": id_test, "relevance": y_pred[:,0]}).to_csv('single_0305_nn_rmse4_1.csv',index=False)

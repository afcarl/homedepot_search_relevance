...
tsvd = TruncatedSVD(n_components = 50, random_state = 8)
...
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('single_0217_3.csv',index=False)
...
def str_common_word(str1, str2):
    str1 = str(str1.encode('utf-8')).lower()
    str2 = str(str2.encode('utf-8')).lower()
    words, cnt = str1.split(), 0
    for word in words:
        if str2.find(word)>=0:
            cnt+=1

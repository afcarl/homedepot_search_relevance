...
clf = pipeline.Pipeline([
    ...
    ('xgbr', xgbr)])
param_grid ={'xgbr__silent': [1], 'xgbr__nthread': [3], 'xgbr__eval_metric': ['rmse'], 'xgbr__eta': [0.01,0.02,0.03],
             'xgbr__max_depth': [6,7,8], 'xgbr__num_round': [600,700], 'xgbr__fit_const': [0.5],
             'xgbr__subsample': [0.66,0.75],'xgbr__objective':['reg:linear']}
...
model = grid_search.GridSearchCV(estimator = clf, param_grid = param_grid, n_jobs = 2, cv = 2, verbose = 0, scoring=RMSE)
...
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('single_0218_rmse3_1.csv',index=False)

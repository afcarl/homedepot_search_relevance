...
df_all.to_csv('df_all4.csv', encoding="ISO-8859-1")
...
xgbr = XGBoostRegressor(silent = 1)
...
clf = pipeline.Pipeline([
    ...
    ('xgbr', xgbr)])
param_grid ={'xgbr__silent': [1], 'xgbr__nthread': [3], 'xgbr__eval_metric': ['rmse'], 'xgbr__eta': [x*0.01 for x in range(10)],
             'xgbr__max_depth': [5,6,7,8,9,10], 'xgbr__num_round': [400,600,700,800,900], 'xgbr__fit_const': [0.4,0.5,0.6,0.7],
             'xgbr__subsample': [0.5,0.66,0.75,0.8],'xgbr__objective':['reg:linear'],'xgbr__gamma':[0,1,5,10,100],
             'xgbr__min_child_weight':[1,3,6,10],'xgbr__colsample_bytree':[0.4,0.5,0.6,0.7]}
# param_grid = {'xgbr__num_boost_round': [100, 250, 500],
#               'xgbr__eta': [0.05, 0.1, 0.3],
#               'xgbr__max_depth': [6, 9, 12],
#               'xgbr__subsample': [0.9, 1.0],
#               'xgbr__colsample_bytree': [0.9, 1.0]}
# param_grid = {'rfr__num_boost_round': [100,250]}
model = grid_search.RandomizedSearchCV(estimator = clf, param_distributions = param_grid, n_jobs = 1, cv = 2, verbose = 0, scoring=RMSE)
...
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('single_0303_rmse3_1.csv',index=False)

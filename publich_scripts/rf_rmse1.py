...
df_train = pd.read_csv('../data/train.csv', encoding="ISO-8859-1")[:500]
df_test = pd.read_csv('../data/test.csv', encoding="ISO-8859-1")[:500]
...
df_all = df_all.drop(['search_term','product_title','product_description','product_info','attr','brand','product_uid','len_of_description', 'len_of_brand','word_in_title','word_in_description','len_of_title','word_in_brand'],axis=1)
print(df_all.head())
...
# mlp = MLPRegressor(random_state = 1, early_stopping = True, hidden_layer_sizes = 5, activation = 'logistic', algorithm = 'l-bfgs', max_iter=25, verbose=True)
#clf = pipeline.Pipeline([('rfr', rfr)])
clf = pipeline.Pipeline([('rfr', rfr)])
param_grid = {}
#param_grid = {'rfr__n_estimators':list(range(34,50,1)), 'rfr__max_depth':list(range(13,15,1))}
model = model_selection.GridSearchCV(estimator = clf, param_grid = param_grid, n_jobs = -1, cv = 10, verbose = 0, scoring=RMSE)
